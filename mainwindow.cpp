...
const std::vector<QString> MainWindow::EnAnswerText = {"Tuna Ice Cream", "Salted Slug","Mouldy Cheese",
                                                       ...
                                                       "A Comfy Dumpster", "On A Pile Of Treasure", "In A Pig Sty",
...
void MainWindow::searchTextEdited(const QString &userSearchInput)
{
    ...
    if(possibleQuestion.size()==1){
        singleMostLikely = possibleQuestion.top();
    }
...
void MainWindow::on_actionFull_Calculator_triggered(){
        ...
        setGeometry(geometry().x(),geometry().y(),750,576);
...
void MainWindow::on_actionMini_mode_triggered(){
        ...
        setGeometry(geometry().x(),geometry().y(),272,213);
        //setSizePolicy(QSizePolicy::Preferred,QSizePolicy::Preferred);
...
void MainWindow::keyPressEvent(QKeyEvent *e){
    if(e->matches(QKeySequence::Undo)){
       on_undoButton_clicked();
        ...
        //prevents focus being shifted to lienEdit when modifier key is pressed
    }
    else if(e->key()==Qt::Key_F1){
        if(singleMostLikely != 100){//correct
            newPosInput(singleMostLikely);
        }
    }
    else if(e->key()==Qt::Key_F2){
        if(singleMostLikely != 100){//correct
            newNegInput(singleMostLikely);
        }
    }
    else{
...
void MainWindow::resizeEvent(QResizeEvent *e){
    ...
    if((e->size().width()<400) || (e->size().height()<350)){
        ui->actionFull_Calculator->setChecked(false);
        ui->actionMini_mode->setChecked(true);
        ui->fullCalcWidget->hide();
        ui->miniCalcWidget->show();
    }else if((e->size().width()>=400) || (e->size().height()>=400)){
        ui->actionMini_mode->setChecked(false);
        ui->actionFull_Calculator->setChecked(true);
        ui->miniCalcWidget->hide();
        ui->fullCalcWidget->show();
    }
    ui->fullCalcWidget->setGeometry(0,0,e->size().width()-18,(e->size().height())-(ui->menuBar->height()));
    ui->miniCalcWidget->setGeometry(0,0,e->size().width()-18,e->size().height()-(ui->menuBar->height()));
    ui->answers->setGeometry(0,0,ui->QA_Widget->geometry().width(),ui->QA_Widget->geometry().height());
    ui->questions->setGeometry(0,0,ui->QA_Widget->geometry().width(),ui->QA_Widget->geometry().height());
    //ui->fullCalcWidget->resize(width(), height());
}

void MainWindow::setInitialSizes(void){
    setGeometry(100,100,750,576);//start in full view
    //ui->resetButton->setSizePolicy();
    //ui->EnRadioButton->sizePolicy(QSizePolicy::Fixed,QSizePolicy::Fixed);
